...
describe('E2E Flow: E-11 Entry', () => {
    ...
    it('should login as a course and assign a teacher in a subject ', () => {
        cy.loginAsUser('cursos', mockOrganization, mockCourse);
    ...
    it('should login as Teacher and create an academic event from schedule', () => {
        cy.loginAsUser('profesores', mockOrganization, mockTeacher);
        cy.get("button").contains("Personalizar calendario").click();
        cy.get('div.tabs-org').contains('Crear evento').click();
        cy.get('select[data-testid="tipo"]').select('Académico');
        cy.get('select[data-testid="categoria"]').select('Examen');
        cy.get('select[data-testid="asignatura"]').select(mockSubject.name);
        cy.get('select[data-testid="grupo"]').select(mockTeacherSchedule.group_name);
        cy.get('input[placeholder="Ingrese el nombre de la actividad"]').type(mockTeacherSchedule.name);
        cy.get('input[type="date"]').type(mockTeacherSchedule.date);
        cy.get('input[type="time"]').first().type(mockTeacherSchedule.start);
        cy.get('input[type="time"]').last().type(mockTeacherSchedule.end);
        cy.get('input[placeholder="Ingrese el espacio reservado"]').type(mockTeacherSchedule.place);
        cy.get('button').contains('Guardar evento').click();
        cy.wait(2000);
    });    

    it('should login as Organization and delete users', () => {
        cy.loginAsOrganization(mockOrganization);
